...
describe("eventToItems — traduction pure SdkEvent → ChatItem", () => {
  it("MessageEvent assistant → bulle assistant streaming:false", () => {
    const items = eventToItems(
      { kind: "MessageEvent", llm_message: { role: "assistant", content: [{ text: "hi " }, { text: "there" }] } },
      3,
    );
    expect(items).toEqual([
      { kind: "assistant", id: "ev-3", text: "hi there", streaming: false, revision: 0 },
    ]);
  });

  it("MessageEvent sans texte → ignoré", () => {
    expect(eventToItems({ kind: "MessageEvent", llm_message: { role: "assistant", content: [] } }, 0)).toEqual([]);
  });
  ...
  it("ObservationEvent → item observation", () => {
    const [item] = eventToItems({ kind: "ObservationEvent", tool_name: "bash", observation: { out: "x" } }, 2);
    expect(item).toMatchObject({ kind: "observation", toolName: "bash", result: { out: "x" } });
  });

  it("AgentErrorEvent → item error", () => {
    expect(eventToItems({ kind: "AgentErrorEvent", error: "boom" }, 4)).toEqual([
      { kind: "error", id: "ev-4", text: "boom" },
    ]);
  });
});
